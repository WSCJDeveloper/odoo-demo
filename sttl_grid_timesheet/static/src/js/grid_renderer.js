...
export class GridRenderer extends Component {
    ...
    async toggleRowTimer(taskId) {
        ...
        if (this.state.rowTimers[taskId]?.running) {
            ...
            if (!taskInfo) return;
            const elapsed = Math.floor((Date.now() - this.state.rowTimers[taskId].startTime) / 1000);
            ...
            if (!this.state.rowTimers[taskId]) {
                ...
                this.state.rowTimers[taskId].startTime = Date.now();
            }


            this.state.rowTimers[taskId].interval = setInterval(() => {
                this.state.rowTimers[taskId].elapsed = Math.floor((Date.now() - this.state.rowTimers[taskId].startTime) / 1000);
            }, 1000);
        }
    }
    ...
    async submitTimerTimesheet() {
        ...
        const unit_amount = parseFloat((elapsedTime / 3600).toFixed(6));
    ...
    formatHours(floatHours) {
        if (floatHours === 0) return "0:00:00";
        const totalSeconds = Math.round(floatHours * 3600);
    ...
    async onUpdateTimesheetHours(ev) {
        ...
        const oldValueStr = input.dataset.oldValue || "0:00:00";
        const date = input.dataset.date;
        ...
        try {
            ...
            // Handle input starting with ":" → treat as minutes (e.g. ":30" => "0:30:00", ":30:45" => "0:30:45")
            if (rawValue.startsWith(":")) {
                rawValue = "0" + rawValue;
            }

            // Case 1: Colon-separated format (HH:MM:SS, H:M:S, MM:SS, etc.)
            if (rawValue.includes(":")) {
                ...
                if (parts.length === 2) {
                    ...
                    if (rawValue.indexOf(":") === rawValue.lastIndexOf(":")) {
                        // Check if first part is likely hours (>=24) or minutes (<60)
            ...
            // Case 2: Decimal hours like "1.5", ".5"
                ...
                const remaining = floatHours - hours;
                const totalSeconds = Math.round(remaining * 3600);
                minutes = Math.floor(totalSeconds / 60);
                seconds = totalSeconds % 60;
            }
            // Case 3: Whole number → treat as total seconds
            else if (/^\d+$/.test(rawValue)) {
                const totalSeconds = parseInt(rawValue, 10);
            ...
            const normalizedTime = `${hours}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
            input.value = normalizedTime;

            // Parse old value (could be in HH:MM or HH:MM:SS format)
            let oldHours = 0;
            if (oldValueStr.includes(":")) {
                const oldParts = oldValueStr.split(":");
                if (oldParts.length >= 2) {
                    oldHours = parseInt(oldParts[0]) || 0;
                    oldHours += (parseInt(oldParts[1]) || 0) / 60;
                    if (oldParts.length === 3) {
                        oldHours += (parseInt(oldParts[2]) || 0) / 3600;
                    }
                }
            } else {
                oldHours = parseFloat(oldValueStr) || 0;
            }

            const newHours = hours + (minutes / 60) + (seconds / 3600);
            const diff = newHours - oldHours;
            const roundedDiff = parseFloat(diff.toFixed(6)); // More precise than Math.round

            if (Math.abs(roundedDiff) < 0.000001) { // Essentially zero
            ...
            this.env.services.notification.add(
                "Invalid format. Use formats like H:M:S, :M:S, HH:MM:SS, H.M, or total seconds", 
                { type: "warning" }
            );
            input.value = oldValueStr;
            return;
    ...
    addTimesheetFromWizard() {
        ...
        const seconds = timeParts[2] ? parseInt(timeParts[2], 10) || 0 : 0;

        // Calculate total hours
        const u_amount = hours + (minutes / 60) + (seconds / 3600);
